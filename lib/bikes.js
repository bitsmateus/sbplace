...
// Ficha técnica: lista de { label, value }. Descarta itens vazios e limita tamanho.
...
// Outras bikes para "você também pode gostar" (mesma condição, sem a atual).
export function listRelatedBikes(bike, limit = 3) {
  return listBikes({ onlyPublic: true, condition: bike.condition || "nova" })
    .filter((b) => b.id !== bike.id && b.status !== "vendida")
    .slice(0, limit);
}
